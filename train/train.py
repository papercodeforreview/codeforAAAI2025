...
def main():
    ...
    data_path = os.path.join(data_root, f'{data_set}/rule_1/sca')
    ...
    nbatch_tensor = torch.tensor([0.5]).cuda()
    optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=weight_decay)
    criterion = nn.BCEWithLogitsLoss(nbatch_tensor)
    ...
    for epoch in range(epochs):
        ...
        msg = "epoch-%d, train_loss-%.4f, train_acc-%.4f, val_loss-%.4f, val_acc-%.4f, val_auroc-%.4f, val_f1_score-%.4f, val_prec-%.4f, val_rec-%.4f, val_ap-%.4f, val_aupr-%.4f" \
              % (epoch, epoch_loss, epoch_acc, val_loss, val_acc, val_auroc, val_f1_score, val_precision, val_recall, val_ap, val_aupr)
        logger.info(msg)
        scheduler.step()
